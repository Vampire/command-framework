...
class LazyReferenceByFunctionTest extends Specification {
    @Subject
    LazyReferenceByFunction<?, ?> testee = new LazyReferenceByFunction<>({ it })
    ...
    def 'constructor should throw exception if value function is null'() {
        when:
            new LazyReferenceByFunction<>(null)
    ...
    def 'equals should return false if both references are already initialized but to different values'() {
        given:
            def random1 = randomUUID()
            def random2 = randomUUID()
            def other = new LazyReferenceByFunction<>({ it })
    ...
    def '#className toString should start with class name'() {
        ...
        where:
            testee                                                    | _
            new LazyReferenceByFunction<>({ null })                   | _
            new LazyReferenceByFunction<Object, Object>({ null }) { } | _
            new LazyReferenceByFunctionSub()                          | _
    ...
    def 'toString should contain field name and value for "#field.name"'() {
        ...
        where:
            field << getAllInstanceFields(Stub(getField(getClass(), 'testee').type))
                    .findAll { !(it.name in ['$spock_interceptor', 'readLock', 'writeLock', 'valueFunction']) }
    }

    static class LazyReferenceByFunctionSub extends LazyReferenceByFunction<Object, Object> {
        LazyReferenceByFunctionSub() {
            super({ null })
        }
    }
}
