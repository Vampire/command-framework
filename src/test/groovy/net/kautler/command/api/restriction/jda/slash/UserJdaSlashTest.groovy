...
class UserJdaSlashTest extends Specification {
    @WeldSetup
    def weld = WeldInitiator
        .from(TestUserJdaSlash)
        .inject(this)
        .build()

    @Inject
    TestUserJdaSlash userJdaSlash
    ...
    def 'user with ID "#expectedUserId" should #be allowed for user with ID "#actualUserId"'() {
        given:
            UserJdaSlash userJdaSlash = Spy(constructorArgs: [expectedUserId])

        and:
            commandContext.message.user.idLong >> actualUserId

        expect:
            userJdaSlash.allowCommand(commandContext) == allowed
    ...
    def 'user with name "#expectedUserName" should #be allowed case-sensitive for user with name "#actualUserName"'() {
        given:
            UserJdaSlash userJdaSlash = Spy(constructorArgs: [expectedUserName])
    ...
    def 'user with name "#expectedUserName" should #be allowed case-insensitive for user with name "#actualUserName"'() {
        given:
            UserJdaSlash userJdaSlash = Spy(constructorArgs: [expectedUserName, false])
    ...
    def 'user with pattern "#expectedUserPattern" should #be allowed for user with name "#actualUserName"'() {
        given:
            UserJdaSlash userJdaSlash = Spy(constructorArgs: [expectedUserPattern])
    ...
    def 'invariant violation [userId: #userId, userName: #userName, caseSensitive: #caseSensitive, userPattern: #userPattern] is checked'() {
        given:
            def userJdaSlashParameters = new UserJdaSlash.Parameters(
                userId, userName, caseSensitive, userPattern)

        when:
            userJdaSlashParameters.ensureInvariants()
    ...
    def 'invariant violation is checked by constructor with #parameterType parameter'() {
        ...
        where:
            ...
            ; { new TestUserJdaSlash(0) }               | 'long'               || ~/One of userId, userName and userPattern should be given/
            ; { new TestUserJdaSlash(null as String) }  | 'String'             || ~/One of userId, userName and userPattern should be given/
            ; { new TestUserJdaSlash(null, true) }      | 'String and boolean' || ~/One of userId, userName and userPattern should be given/
            ; { new TestUserJdaSlash(null as Pattern) } | 'Pattern'            || ~/One of userId, userName and userPattern should be given/
    }

    @ApplicationScoped
    private static class TestUserJdaSlash extends UserJdaSlash {
        TestUserJdaSlash() {
            super(-1)
        }

        TestUserJdaSlash(long userId) {
            super(userId)
        }

        TestUserJdaSlash(String userName) {
            super(userName as String)
        }

        TestUserJdaSlash(String userName, boolean caseSensitive) {
            super(userName, caseSensitive)
        }

        TestUserJdaSlash(Pattern userPattern) {
