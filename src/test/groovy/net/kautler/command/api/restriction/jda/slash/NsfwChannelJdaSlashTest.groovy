...
@EnableWeld
class NsfwChannelJdaSlashTest extends Specification {
    @WeldSetup
    def weld = WeldInitiator
            .from(NsfwChannelJdaSlash)
            .inject(this)
            .build()

    @Inject
    @Subject
    NsfwChannelJdaSlash nsfwChannelJdaSlash
    ...
    def 'nsfw channel "#nsfw" should #be allowed'() {
        ...
        expect:
            nsfwChannelJdaSlash.allowCommand(commandContext) == allowed
    ...
    def 'non-age-restricted channel should not be allowed'() {
        ...
        expect:
            !nsfwChannelJdaSlash.allowCommand(commandContext)
    }
}
