...
class AllOfTest extends Specification {
    Restriction<Object> restriction1 = Stub()

    Restriction<Object> restriction2 = Stub()

    @Subject
    AllOf<Object> allOf = Spy(constructorArgs: [restriction1, restriction2])

    def 'all of [restriction1Allowed: #restriction1Allowed, restriction2Allowed: #restriction2Allowed] should #be allowed'() {
        given:
            restriction1.allowCommand(_) >> restriction1Allowed
            restriction2.allowCommand(_) >> restriction2Allowed

        expect:
            allOf.allowCommand(Stub(CommandContext)) == allowed
