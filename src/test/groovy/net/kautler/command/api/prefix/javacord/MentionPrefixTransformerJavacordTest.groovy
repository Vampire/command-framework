...
class MentionPrefixTransformerJavacordTest extends Specification {
    Message message = Stub {
        it.api >> Stub(DiscordApi) {
            it.yourself >> Stub(User) {
                it.mentionTag >> '<@12345>'
                it.nicknameMentionTag >> '<@!12345>'
            }
        }
    }

    @Subject
    MentionPrefixTransformerJavacord testee = Spy()
    ...
    def '#className toString should start with class name'() {
        ...
        where:
            testee                                     | _
            Spy(MentionPrefixTransformerJavacord)      | _
            new MentionPrefixTransformerJavacord() { } | _
            new MentionPrefixTransformerJavacordSub()  | _
