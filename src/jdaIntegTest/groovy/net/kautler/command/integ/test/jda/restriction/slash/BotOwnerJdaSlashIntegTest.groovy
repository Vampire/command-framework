...
import net.kautler.command.api.restriction.jda.slash.BotOwnerJdaSlash
...
@Subject(BotOwnerJdaSlash)
@Tag('manual')
@AddBean(SlashCommandRegisterer)
class BotOwnerJdaSlashIntegTest extends DiscordGebSpec {
    @AddBean(PingCommand)
    @AddBean(IgnoreOtherTestsTransformer)
    @ResourceLock('net.kautler.command.integ.test.jda.restriction.slash.BotOwnerJdaSlashIntegTest.PingCommand.alias')
    @ResourceLock('net.kautler.command.integ.test.jda.restriction.slash.BotOwnerJdaSlashIntegTest.PingCommand.commandNotAllowedEventReceived')
    @ResourceLock('net.kautler.command.integ.test.jda.restriction.slash.BotOwnerJdaSlashIntegTest.IgnoreOtherTestsTransformer.expectedContent')
    def 'ping command should not respond if not bot owner'(TextChannel textChannelAsBot) {
        given:
            IgnoreOtherTestsTransformer.expectedContent = "/${PingCommand.alias}"
            def commandNotAllowedEventReceived = new BlockingVariable<Boolean>(System.properties.testResponseTimeout as double)
            PingCommand.commandNotAllowedEventReceived = commandNotAllowedEventReceived
            assert System.properties.testDiscordUserId != textChannelAsBot.JDA.retrieveApplicationInfo().complete().owner.id
    ...
    @ResourceLock('net.kautler.command.integ.test.jda.restriction.slash.BotOwnerJdaSlashIntegTest.PingCommand.alias')
    @ResourceLock('net.kautler.command.integ.test.jda.restriction.slash.BotOwnerJdaSlashIntegTest.IgnoreOtherTestsTransformer.expectedContent')
    def 'ping command should respond if bot owner'(TextChannel textChannelAsBot) {
        given:
            IgnoreOtherTestsTransformer.expectedContent = "/${PingCommand.alias}"
        ...
        and:
            ...
            subscriptions << textChannelAsBot
                    .JDA
                    .listenOnce(SlashCommandInteractionEvent)
                    .filter { it.interaction.user == owner }
                    .filter { it.interaction.channel == textChannelAsBot }
                    .filter { it.interaction.name == PingCommand.alias }
                    .filter { !it.interaction.options }
                    .subscribe { commandReceived.set(true) }
