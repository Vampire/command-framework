 ...
 */
public abstract class ChannelJavacordSlash implements Restriction<SlashCommandInteraction> {
    /**
     ...
     */
    protected ChannelJavacordSlash(long channelId) {
     ...
     */
    protected ChannelJavacordSlash(String channelName) {
     ...
     */
    protected ChannelJavacordSlash(String channelName, boolean caseSensitive) {
     ...
     */
    protected ChannelJavacordSlash(Pattern channelPattern) {
     ...
     */
    private ChannelJavacordSlash(Parameters parameters) {
    ...
    public boolean allowCommand(CommandContext<? extends SlashCommandInteraction> commandContext) {
        TextChannel textChannel = commandContext
                .getMessage()
                .getChannel()
                .orElseThrow(AssertionError::new);
        return ((channelName == null) && (channelPattern == null))
               ? allowCommandByChannelId(textChannel)
               : allowCommandByChannelName(textChannel);
    }

    /**
     * Returns whether a command is allowed according to the configured channel ID.
     *
     * @param textChannel the text channel of the command to check
     * @return whether a command is allowed according to the configured channel ID
     */
    private boolean allowCommandByChannelId(TextChannel textChannel) {
        return textChannel.getId() == channelId;
    }
     ...
     */
    private boolean allowCommandByChannelName(TextChannel textChannel) {
        return textChannel
                .asServerChannel()
    ...
    /**
     * A set of parameters to construct a channel restriction for Javacord with slash commands.
     */
    private static class Parameters {
         ...
         *                       to determine where a command should be allowed
         */
        private Parameters(long channelId, String channelName,
