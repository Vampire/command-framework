...
/**
 * A restriction that allows a command for NSFW channels and is evaluated by the JDA slash command handler.
 * If a message is not sent on a guild, this restriction always denies.
 */
@ApplicationScoped
public class NsfwChannelJdaSlash implements Restriction<SlashCommandInteraction> {
    /**
     * Constructs a new NSFW channel restriction for JDA slash commands.
     */
    public NsfwChannelJdaSlash() {
    ...
    public boolean allowCommand(CommandContext<? extends SlashCommandInteraction> commandContext) {
        return Optional.of(commandContext.getMessage())
                .map(SlashCommandInteraction::getChannel)
