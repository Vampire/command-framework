 ...
 */
public abstract class RoleJavacordSlash implements Restriction<SlashCommandInteraction> {
    /**
     ...
     */
    protected RoleJavacordSlash(long roleId) {
     ...
     */
    protected RoleJavacordSlash(String roleName) {
     ...
     */
    protected RoleJavacordSlash(String roleName, boolean caseSensitive) {
     ...
     */
    protected RoleJavacordSlash(Pattern rolePattern) {
     ...
     * @param roleId the ID of the role for which a command should be allowed
     */
    protected RoleJavacordSlash(boolean exact, long roleId) {
     ...
     * @param roleName the case-sensitive name of the role for which a command should be allowed
     */
    protected RoleJavacordSlash(boolean exact, String roleName) {
     ...
     * @param caseSensitive whether the name should be matched case-sensitively or not
     */
    protected RoleJavacordSlash(boolean exact, String roleName, boolean caseSensitive) {
     ...
     *                    to determine for whom a command should be allowed
     */
    protected RoleJavacordSlash(boolean exact, Pattern rolePattern) {
     ...
     */
    private RoleJavacordSlash(Parameters parameters) {
    ...
    private boolean allowCommandByExactRoleId(SlashCommandInteraction slashCommandInteraction) {
        return slashCommandInteraction
                .getServer()
                .map(server -> server.getRoles(slashCommandInteraction.getUser()))
    ...
    private boolean allowCommandByAtLeastRoleId(SlashCommandInteraction slashCommandInteraction) {
        return slashCommandInteraction
                ...
                .flatMap(server -> server.getRoleById(roleId)
                        .flatMap(role -> server
                                .getHighestRole(slashCommandInteraction.getUser())
    ...
    /**
     * A set of parameters to construct a role restriction for Javacord with slash commands.
     */
