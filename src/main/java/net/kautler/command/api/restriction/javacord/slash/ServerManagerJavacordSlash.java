 ...
 */
@ApplicationScoped
public class ServerManagerJavacordSlash implements Restriction<SlashCommandInteraction> {
    /**
     * Constructs a new server manager restriction for Javacord slash commands.
     */
    public ServerManagerJavacordSlash() {
    ...
    public boolean allowCommand(CommandContext<? extends SlashCommandInteraction> commandContext) {
        ...
        return slashCommandInteraction
                .getServer()
                .map(server -> server.canManage(slashCommandInteraction.getUser()))
                .orElse(FALSE);
    }
}
